...
describe('getContextWindowFallback', () => {
  ...
  test('returns exact match for known models', async () => {
    assert.equal(getContextWindowFallback('claude-opus-4-6'), 1_000_000);
    ...
    assert.equal(getContextWindowFallback('glm-5.2[1m]'), 1_000_000);
  ...
  test('returns prefix match for versioned models', async () => {
    assert.equal(getContextWindowFallback('claude-opus-4-6-20260101'), 1_000_000);
    assert.equal(getContextWindowFallback('claude-sonnet-4-6-20260101'), 1_000_000);
  ...
  test('clowder#915: strips provider prefix for account-routing model IDs', async () => {
    ...
    assert.equal(getContextWindowFallback('anthropic/claude-opus-4-6'), 1_000_000);
  ...
  test('clowder#915: handles multi-segment prefix defensively (last segment wins)', async () => {
    ...
    assert.equal(getContextWindowFallback('anthropic/v1/claude-opus-4-6'), 1_000_000);
    assert.equal(getContextWindowFallback('anthropic/v1/claude-sonnet-4-6'), 1_000_000);
  });
...
describe('getKnownMinContextWindow / resolveContextWindow', () => {
  ...
  test('resolveContextWindow never shrinks a CLI report', async () => {
    ...
    // no authoritative floor → passthrough stale CLI report as-is
    assert.equal(resolveContextWindow(200_000, 'claude-sonnet-4-5'), 200_000);
  });

  test('resolveContextWindow falls back to the table; floor still applies', async () => {
    assert.equal(resolveContextWindow(undefined, 'claude-opus-4-6'), 1_000_000);
    assert.equal(resolveContextWindow(undefined, 'claude-sonnet-4-6'), 1_000_000);
    // Without the floor, the [1m] form would prefix-match to the bare
    // model's table entry; the floor still corrects it to 1M.
